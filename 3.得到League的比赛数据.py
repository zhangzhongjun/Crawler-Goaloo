...
baseUrl = 'http://info.goaloo.com/jsData/matchResult/%s/s%s.js'
...
for s in lines:
    ...
    if words[3] != u'1':
        continue
    path = 'F://goaloo//ju_League//'+words[0]+u"_"+words[2]+'.csv'
                    ...
                    for d in data:
                        if str(type(d[4]))=="<type 'list'>":
                            for i in range(4,len(d)):
                                ...
                                try:
                                    if bisai[6].find('-')!=-1:
                                        res=[words[0],words[1],words[2],words[3],season,u"",round,bisai[3],teams[bisai[4]],teams[bisai[5]],bisai[6].split('-')[0],bisai[6].split('-')[1]]
                                    else:
                                        res=[words[0],words[1],words[2],words[3],season,u"",round,bisai[3],teams[bisai[4]],teams[bisai[5]],u'cancel',u'cancel']
                                    cu.collect(res)
                            ...
                            for d in data:
                                ...
                                try:
                                    if d[6].find('-')!=-1:
                                        res=[words[0],words[1],words[2],words[3],season,u"",round,d[3],teams[d[4]],teams[d[5]],d[6].split('-')[0],d[6].split('-')[1]]
                                    else:
                                        res=[words[0],words[1],words[2],words[3],season,u"",round,d[3],teams[d[4]],teams[d[5]],u'cancel',u'cancel']
                                    cu.collect(res)
                                except Exception as e:
